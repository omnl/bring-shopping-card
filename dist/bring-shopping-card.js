...
`,ut={show_quick_add:"Show Quick Add",show_quick_add_description:"Display recently purchased items for quick re-adding",show_all_items:"Show All Items",show_all_items_description:"Display all available items grouped by category",max_quick_add_items:"Max Quick Add Items",max_quick_add_items_description:"Maximum items to show in Quick Add section",card_size:"Card Size",card_size_description:"Size of the product item cards",small:"Small",medium:"Medium",large:"Large",no_shopping_lists:"No shopping lists found",failed_to_connect:"Failed to connect to Bring! integration",failed_to_load_list:"Failed to load shopping list",added:"Added {name}",failed_to_add:"Failed to add item",done:"Done: {name}",failed_to_complete:"Failed to complete item",remove:"Remove",clear_list:"Clear list",confirm_clear_list:"Sure?",removed:"Removed: {name}",failed_to_remove:"Failed to remove item",updated:"Updated",failed_to_update:"Failed to update",shopping_card:"Shopping Card",select_list:"Select List",list:"List",sort:"Sort",manual_order:"Manual Order",by_category:"By Category",recently_added:"Recently Added",refresh:"Refresh",add_item_placeholder:"Add item...",add:"Add",to_buy:"To Buy",list_empty:"List is empty!",empty_list_hint:"Add items above or tap below",add_note:"+ note",quick_add:"Quick Add",other:"Other",all_items:"All Items",edit:"Edit: {name}",specification_placeholder:"e.g., 2 lbs, organic",cancel:"Cancel",save:"Save"},_t={en:ut,de:{show_quick_add:"Schnell hinzufügen anzeigen",show_quick_add_description:"Kürzlich gekaufte Artikel zum schnellen erneuten Hinzufügen anzeigen",show_all_items:"Alle Artikel anzeigen",show_all_items_description:"Alle verfügbaren Artikel nach Kategorie gruppiert anzeigen",max_quick_add_items:"Maximale Anzahl für Schnell hinzufügen",max_quick_add_items_description:"Maximale Anzahl der Artikel im Bereich „Schnell hinzufügen“",card_size:"Kartengröße",card_size_description:"Größe der Produktkarten",small:"Klein",medium:"Mittel",large:"Groß",no_shopping_lists:"Keine Einkaufslisten gefunden",failed_to_connect:"Verbindung zur Bring!-Integration fehlgeschlagen",failed_to_load_list:"Einkaufsliste konnte nicht geladen werden",added:"{name} hinzugefügt",failed_to_add:"Artikel konnte nicht hinzugefügt werden",done:"Erledigt: {name}",failed_to_complete:"Artikel konnte nicht abgeschlossen werden",remove:"Entfernen",clear_list:"Liste leeren",confirm_clear_list:"Sicher?",removed:"{name} entfernt",failed_to_remove:"Artikel konnte nicht entfernt werden",updated:"Aktualisiert",failed_to_update:"Aktualisierung fehlgeschlagen",shopping_card:"Einkaufskarte",select_list:"Liste auswählen",list:"Liste",sort:"Sortieren",manual_order:"Manuelle Reihenfolge",by_category:"Nach Kategorie",recently_added:"Kürzlich hinzugefügt",refresh:"Aktualisieren",add_item_placeholder:"Artikel hinzufügen …",add:"Hinzufügen",to_buy:"Zu kaufen",list_empty:"Die Liste ist leer!",empty_list_hint:"Füge oben Artikel hinzu oder tippe unten auf einen Artikel",add_note:"+ Notiz",quick_add:"Schnell hinzufügen",other:"Sonstiges",all_items:"Alle Artikel",edit:"Bearbeiten: {name}",specification_placeholder:"z. B. 2 kg, Bio",cancel:"Abbrechen",save:"Speichern"}};function bt(t,e,i={}){const s=t?.toLowerCase().split("-")[0]??"en";return((_t[s]??ut)[e]??ut[e]).replace(/\{(\w+)\}/g,(t,e)=>void 0===i[e]?t:String(i[e]))}const ft=(t,e)=>`bring-shopping-card-${t}${e?`-${e}`:""}`;let vt=class extends nt{setConfig(t){this._config=t}_valueChanged(t,e){if(!this._config)return;const i={...this._config,[t]:e},s=new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0});this.dispatchEvent(s)}_t(t){return bt(this.hass?.locale?.language??this.hass?.language,t)}render(){return this._config?B`
  ...
  `,t([pt({attribute:!1})],vt.prototype,"hass",void 0),t([gt()],vt.prototype,"_config",void 0),vt=t([lt("bring-shopping-card-editor")],vt);let xt=class extends nt{constructor(){super(...arguments),this._lists=[],this._selectedListUuid=null,this._items=[],this._recentItems=[],this._availableItems=[],this._sortBy="manual",this._customOrder=[],this._searchQuery="",this._selectedSuggestion=-1,this._loading=!0,this._error=null,this._editingItem=null,this._showSortMenu=!1,this._showListDropdown=!1,this._showSuggestions=!1,this._confirmClear=!1,this._failedImages=new Set,this._pendingCompletions=new Set,this._pendingCompletionItems=new Map,this._pendingAdditions=new Map,this._pendingSpecifications=new Map,this._completionTimers=new Map,this._fetchSequence=0,this._suppressCardClickUntil=0,this._cardKey="default",this._draggedItem=null}_t(t,e={}){return bt(this.hass?.locale?.language??this.hass?.language,t,e)}setConfig(t){if(!t)throw new Error("Invalid configuration");this.config={show_recently:!1,show_available:!1,max_quick_items:12,sort_default:"manual",card_size:"medium",...t},this._cardKey=this.config.card_id||"default",this._sortBy=this.config.sort_default||"manual",this.dataset.size=this.config.card_size||"medium"}updated(t){t.has("hass")&&(this.dataset.theme=this.hass?.themes?.darkMode?"dark":"light")}static getConfigElement(){return document.createElement("bring-shopping-card-editor")}static getStubConfig(){return{type:"custom:bring-shopping-card"}}getCardSize(){return 5}connectedCallback(){super.connectedCallback(),this._loadSavedState(),this._fetchLists(),this._startAutoRefresh()}disconnectedCallback(){super.disconnectedCallback(),this._stopAutoRefresh()}_loadSavedState(){try{const t=localStorage.getItem(ft("list",this._cardKey));t&&(this._selectedListUuid=t);const e=localStorage.getItem(ft("sort",this._cardKey));e&&(this._sortBy=e);const i=localStorage.getItem(ft("order",this._cardKey));i&&(this._customOrder=JSON.parse(i))}catch(t){console.error("Failed to load saved state:",t)}}_saveState(){try{this._selectedListUuid&&localStorage.setItem(ft("list",this._cardKey),this._selectedListUuid),localStorage.setItem(ft("sort",this._cardKey),this._sortBy),localStorage.setItem(ft("order",this._cardKey),JSON.stringify(this._customOrder))}catch(t){console.error("Failed to save state:",t)}}async _fetchLists(){try{const t=await this.hass.callWS({type:"bring_shopping/get_lists"});this._lists=t.lists,this._lists.length>0?(this._selectedListUuid&&this._lists.find(t=>t.uuid===this._selectedListUuid)||(this._selectedListUuid=this._lists[0].uuid),await this._fetchItems()):(this._loading=!1,this._error=this._t("no_shopping_lists"))}catch(t){console.error("Failed to fetch lists:",t),this._loading=!1,this._error=this._t("failed_to_connect")}}async _fetchItems(){if(!this._selectedListUuid)return;const t=this._selectedListUuid,e=++this._fetchSequence;try{const i=await this.hass.callWS({type:"bring_shopping/get_items",list_uuid:t});if(e!==this._fetchSequence||t!==this._selectedListUuid)return;const s=new Set(i.purchase.map(t=>t.originalName));for(const e of[...this._pendingCompletions]){const r=e.slice(t.length+1),a=!s.has(r)&&i.recently.some(t=>t.originalName===r)&&i.available.some(t=>t.originalName===r);if(e.startsWith(`${t}:`)&&a){this._pendingCompletions.delete(e),this._pendingCompletionItems.delete(e);const t=this._completionTimers.get(e);t&&clearTimeout(t),this._completionTimers.delete(e)}}let r=i.purchase.filter(e=>!this._pendingCompletions.has(`${t}:${e.originalName}`));const a=new Set(r.map(t=>t.originalName));for(const[e,i]of[...this._pendingAdditions])e.startsWith(`${t}:`)&&(a.has(i.originalName)?this._pendingAdditions.delete(e):r=[...r,i]);r=r.map(e=>{const i=`${t}:${e.originalName}`,s=this._pendingSpecifications.get(i);return void 0===s?e:(e.specification===s&&this._pendingSpecifications.delete(i),{...e,specification:s})});let o=i.recently.filter(e=>!this._pendingAdditions.has(`${t}:${e.originalName}`)),n=i.available.filter(e=>!this._pendingAdditions.has(`${t}:${e.originalName}`));for(const[e,i]of this._pendingCompletionItems)e.startsWith(`${t}:`)&&(o.some(t=>t.originalName===i.originalName)||(o=[i,...o]),n.some(t=>t.originalName===i.originalName)||(n=[i,...n]));this._items=r,this._recentItems=o,this._availableItems=n,this._loading=!1,this._error=null}catch(t){console.error("Failed to fetch items:",t),this._loading=!1,0===this._items.length?this._error=this._t("failed_to_load_list"):this._showToast(this._t("failed_to_load_list"),"error")}}_startAutoRefresh(){this._refreshInterval=window.setInterval(()=>{document.hidden||this._fetchItems()},6e4)}_stopAutoRefresh(){this._refreshInterval&&clearInterval(this._refreshInterval)}_getSortedItems(){const t=[...this._items];switch(this._sortBy){case"manual":const e={};return this._customOrder.forEach((t,i)=>e[t]=i),t.sort((t,i)=>(e[t.originalName]??999)-(e[i.originalName]??999));case"alpha":return t.sort((t,e)=>t.name.localeCompare(e.name));case"category":return t.sort((t,e)=>{const i=t.category||"ZZZ",s=e.category||"ZZZ";return i!==s?i.localeCompare(s):t.name.localeCompare(e.name)});default:return t}}_getFilteredSuggestions(){if(!this._searchQuery.trim())return[];const t=this._searchQuery.toLowerCase(),e=this._availableItems.filter(e=>e.name.toLowerCase().includes(t)||e.originalName.toLowerCase().includes(t)),i=this._recentItems.filter(e=>(e.name.toLowerCase().includes(t)||e.originalName.toLowerCase().includes(t))&&!this._items.some(t=>t.originalName===e.originalName)),s=[...e];return i.forEach(t=>{s.some(e=>e.originalName===t.originalName)||s.push(t)}),s.slice(0,8)}async _addItem(t,e,i=""){if(!t.trim()||!this._selectedListUuid)return;const s=this._selectedListUuid,r=t.trim(),a=e||r,o=`${s}:${a}`;if(this._pendingAdditions.has(o))return;const n=this._availableItems.find(t=>t.originalName===a),d=this._recentItems.find(t=>t.originalName===a),l=n||d,c=l?{...l,name:r,specification:i}:{name:r,originalName:a,specification:i,icon:"🛒",imageUrl:null,category:this._t("other")};this._pendingAdditions.set(o,c),this._items=[...this._items,c],this._availableItems=this._availableItems.filter(t=>t.originalName!==a),this._recentItems=this._recentItems.filter(t=>t.originalName!==a);try{await this.hass.callWS({type:"bring_shopping/add_item",list_uuid:s,item_name:r,original_name:a,specification:i}),this._showToast(this._t("added",{name:t}),"success"),await this._fetchItems()}catch(t){console.error("Failed to add item:",t),this._pendingAdditions.delete(o),this._selectedListUuid===s&&(this._items=this._items.filter(t=>t.originalName!==a),n&&!this._availableItems.some(t=>t.originalName===a)&&(this._availableItems=[...this._availableItems,n]),d&&!this._recentItems.some(t=>t.originalName===a)&&(this._recentItems=[...this._recentItems,d])),this._showToast(this._t("failed_to_add"),"error")}}async _completeItem(t){if(!this._selectedListUuid)return;const e=this._selectedListUuid,i=`${e}:${t.originalName}`;if(this._pendingCompletions.has(i))return;const s=this._items.findIndex(e=>e.originalName===t.originalName);this._pendingCompletions.add(i),this._pendingCompletionItems.set(i,t),this._items=this._items.filter(e=>e.originalName!==t.originalName),this._recentItems.some(e=>e.originalName===t.originalName)||(this._recentItems=[t,...this._recentItems]),this._availableItems.some(e=>e.originalName===t.originalName)||(this._availableItems=[t,...this._availableItems]),this._showToast(this._t("done",{name:t.name}),"success");try{await this.hass.callWS({type:"bring_shopping/complete_item",list_uuid:e,original_name:t.originalName});const s=window.setTimeout(()=>{this._pendingCompletions.delete(i),this._pendingCompletionItems.delete(i),this._completionTimers.delete(i),this._fetchItems()},15e3);this._completionTimers.set(i,s)}catch(r){console.error("Failed to complete item:",r),this._pendingCompletions.delete(i),this._pendingCompletionItems.delete(i),this._selectedListUuid!==e||this._items.some(e=>e.originalName===t.originalName)||(this._items=[...this._items.slice(0,Math.max(0,s)),t,...this._items.slice(Math.max(0,s))]),this._showToast(this._t("failed_to_complete"),"error")}}async _clearList(){if(!this._selectedListUuid||!this._items.length)return;if(!this._confirmClear)return this._confirmClear=!0,void window.setTimeout(()=>this._confirmClear=!1,4e3);this._confirmClear=!1;const t=this._selectedListUuid,e=this._items;e.forEach(e=>{const i=`${t}:${e.originalName}`;this._pendingCompletions.add(i),this._pendingCompletionItems.set(i,e)}),this._items=[],this._recentItems=[...e.filter(t=>!this._recentItems.some(e=>e.originalName===t.originalName)),...this._recentItems],this._availableItems=[...e.filter(t=>!this._availableItems.some(e=>e.originalName===t.originalName)),...this._availableItems];const i=await Promise.allSettled(e.map(e=>this.hass.callWS({type:"bring_shopping/complete_item",list_uuid:t,original_name:e.originalName}))),s=e.filter((t,e)=>"rejected"===i[e].status);s.forEach(e=>{const i=`${t}:${e.originalName}`;this._pendingCompletions.delete(i),this._pendingCompletionItems.delete(i)}),this._selectedListUuid===t&&s.length&&(this._items=[...this._items,...s],this._recentItems=this._recentItems.filter(t=>!s.some(e=>e.originalName===t.originalName)),this._availableItems=this._availableItems.filter(t=>!s.some(e=>e.originalName===t.originalName)),this._showToast(this._t("failed_to_remove"),"error")),window.setTimeout(()=>this._fetchItems(),15e3)}async _updateItemSpec(t,e){if(!this._selectedListUuid)return;const i=this._selectedListUuid,s=`${i}:${t.originalName}`,r=t.specification;this._pendingSpecifications.set(s,e),this._items=this._items.map(i=>i.originalName===t.originalName?{...i,specification:e}:i);try{await this.hass.callWS({type:"bring_shopping/update_item",list_uuid:i,original_name:t.originalName,specification:e}),this._showToast(this._t("updated"),"success"),await this._fetchItems()}catch(e){console.error("Failed to update item:",e),this._pendingSpecifications.delete(s),this._selectedListUuid===i&&(this._items=this._items.map(e=>e.originalName===t.originalName?{...e,specification:r}:e)),this._showToast(this._t("failed_to_update"),"error")}}_showToast(t,e="success"){const i=this.shadowRoot?.querySelector(".toast-container");if(!i)return;const s=document.createElement("div");s.className=`toast ${e}`,s.textContent=t,i.appendChild(s),setTimeout(()=>{s.style.animation="toastOut 0.3s ease forwards",setTimeout(()=>s.remove(),300)},2e3)}_handleInputKeydown(t){const e=this._getFilteredSuggestions(),i=t.target;if("ArrowDown"===t.key)t.preventDefault(),this._selectedSuggestion=Math.min(this._selectedSuggestion+1,e.length-1);else if("ArrowUp"===t.key)t.preventDefault(),this._selectedSuggestion=Math.max(this._selectedSuggestion-1,-1);else if("Enter"===t.key)if(t.preventDefault(),this._selectedSuggestion>=0&&e[this._selectedSuggestion]){const t=e[this._selectedSuggestion];this._addItem(t.name,t.originalName),i.value="",this._searchQuery="",this._showSuggestions=!1}else i.value.trim()&&(this._addItem(i.value),i.value="",this._searchQuery="",this._showSuggestions=!1);else"Escape"===t.key&&(this._showSuggestions=!1)}_handleInputChange(t){const e=t.target;this._searchQuery=e.value,this._showSuggestions=e.value.length>0,this._selectedSuggestion=-1}_handleSuggestionClick(t){this._addItem(t.name,t.originalName);const e=this.shadowRoot?.querySelector(".add-input");e&&(e.value=""),this._searchQuery="",this._showSuggestions=!1}_handleDragStart(t,e){this._draggedItem=e,t.target.classList.add("dragging"),t.dataTransfer.effectAllowed="move",t.dataTransfer.setData("text/plain",e.originalName)}_handleDragEnd(t){t.target.classList.remove("dragging"),this.shadowRoot?.querySelectorAll(".card.drag-over").forEach(t=>{t.classList.remove("drag-over")}),this._draggedItem=null,this._suppressCardClickUntil=Date.now()+250}_handleDragOver(t){t.preventDefault(),t.dataTransfer.dropEffect="move"}_handleDragEnter(t,e){e.classList.add("drag-over")}_handleDragLeave(t,e){e.classList.remove("drag-over")}_handleDrop(t,e,i){if(t.preventDefault(),i.classList.remove("drag-over"),!this._draggedItem||this._draggedItem.originalName===e.originalName)return;const s=this._getSortedItems().map(t=>t.originalName),r=this._draggedItem.originalName,a=s.filter(t=>t!==r),o=a.indexOf(e.originalName);a.splice(Math.max(0,o),0,r),this._customOrder=a,"manual"!==this._sortBy&&(this._sortBy="manual"),this._saveState()}_selectList(t){this._selectedListUuid=t.uuid,this._showListDropdown=!1,this._loading=!0,this._items=[],this._saveState(),this._fetchItems()}_renderImage(t,e){const i="large"===e?"card-img":"quick-card-img",s="large"===e?"card-initial":"quick-card-initial";return t.imageUrl&&!this._failedImages.has(t.imageUrl)?B`
        <img
          class="${i}"
          src="${t.imageUrl}"
          alt="${t.name}"
        ...
        />
      `:B`<span class="${s}" aria-hidden="true">${t.name.trim().charAt(0).toUpperCase()||"?"}</span>`}_renderHeader(){const t=this._lists.find(t=>t.uuid===this._selectedListUuid);return B`
      <header class="header">
        <div class="header-actions">
          ...
          <button
            class="header-btn"
            @click=${async t=>{t.stopPropagation();const e=t.currentTarget;e.classList.add("spinning"),0===this._lists.length?await this._fetchLists():await this._fetchItems(),e.classList.remove("spinning")}}
            ...
            ${t.map((t,e)=>B`
                ...
                >
                  ${t.imageUrl&&!this._failedImages.has(t.imageUrl)?B`
                        ...
                        />
                      `:B`<span class="suggestion-initial" aria-hidden="true">${t.name.trim().charAt(0).toUpperCase()||"?"}</span>`}
                  <span class="suggestion-text">${t.name}</span>
                  ${t.category?B`<span class="suggestion-category">${t.category}</span>`:F}
                </div>
              `)}
          </div>
        </div>
              ...
              <div class="cards-grid">
                ${t.map(t=>B`
                    <div
                      class="card"
                      draggable="true"
                      @click=${()=>{Date.now()>=this._suppressCardClickUntil&&this._completeItem(t)}}
                    ...
                    >
                      <div class="card-content">
                        ...
                        ${t.category?B`<span class="card-category">${t.category}</span>`:F}
                      </div>
                    </div>
                  `)}
              </div>
    ...
    `}_renderQuickAdd(){if(!this.config.show_recently||0===this._recentItems.length)return F;const t=this._recentItems.slice(0,this.config.max_quick_items||12);return B`
      <section class="section">
        <div class="section-header">
    ...
    `}_renderAvailableItems(){if(!this.config.show_available||0===this._availableItems.length)return F;const t={};this._availableItems.forEach(e=>{const i=e.category||this._t("other");t[i]||(t[i]=[]),t[i].push(e)});const e=Object.keys(t).sort();return B`
    ...
    `:F}render(){const t=t=>{const e=t.target;e.closest(".list-selector-wrapper")||e.closest(".sort-dropdown")||(this._showSortMenu=!1,this._showListDropdown=!1)};return this._loading?B`
        <ha-card @click=${t}>
          <div class="container">
            ${this._renderHeader()}
            <div class="loading">
